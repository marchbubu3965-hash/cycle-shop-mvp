...
//                     id: row.order_id, 
//                     date: row.created_at, 
//                     total: row.final_amount + row.wallet_used, 
...
// 買家商品列表：增加 Log 確保知道抓到幾筆
app.get('/api/products', (req, res) => {
    const sql = "SELECT * FROM Products WHERE status = 'active' AND stock > 0 ORDER BY product_id DESC";
    db.all(sql, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        console.log(`[Buyer API] 發送商品數量: ${rows.length}`);
        res.json(rows);
    });
});
...
app.get('/api/user/:id/orders', (req, res) => {
    const sql = `SELECT o.order_id, o.created_at, o.final_amount, o.wallet_used, oi.item_id, p.name, p.price, oi.buyback_status, p.description FROM Orders o JOIN Order_Items oi ON o.order_id = oi.order_id JOIN Products p ON oi.product_id = p.product_id WHERE o.user_id = ? ORDER BY o.created_at DESC`;
    db.all(sql, [req.params.id], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        const orders = rows.reduce((acc, row) => {
            if (!acc[row.order_id]) acc[row.order_id] = { id: row.order_id, date: row.created_at, total: row.final_amount + row.wallet_used, items: [] };
            acc[row.order_id].items.push(row);
            return acc;
        }, {});
        res.json(Object.values(orders));
